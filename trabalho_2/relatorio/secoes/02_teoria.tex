...
entre a saída e um sinal desejado. No domínio da frequência, para processos
estacionários de média nula, a resposta em frequência ótima é
\begin{equation}
H(e^{j\omega}) = \frac{S_{xy}(e^{j\omega})}{S_{xx}(e^{j\omega})},
\end{equation}
em que \(S_{xx}\) é a densidade espectral da observação e \(S_{xy}\) é o
espectro cruzado entre observação e sinal desejado. Quando o sinal desejado e o
ruído são incorrelacionados e \(y = x + r\), obtém-se a forma
\begin{equation}
H(e^{j\omega}) = \frac{S_{xx}(e^{j\omega})}{S_{xx}(e^{j\omega}) + S_{rr}(e^{j\omega})}.
\end{equation}
No presente trabalho, \(S_{xx}\) é modelada a partir do espectro do template
(escalado por uma amplitude típica do treino) e \(S_{rr}\) é estimada pelo
método de Welch nas janelas de ruído puro.
...
Quando o ruído é colorido, a formulação ótima envolve a branqueação do ruído
